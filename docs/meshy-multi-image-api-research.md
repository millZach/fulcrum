...
Checked against Meshy's official API documentation on 2026-08-25.
...
Enable the existing Fulcrum multiview profile for both `meshy-6` and `meshy-7`. Add `remove_lighting: true` only to the Meshy-6 multi-image request, mirroring the existing Meshy-6 single-image branch. No other profile field needs to differ for Meshy-6 based on the current endpoint reference.
